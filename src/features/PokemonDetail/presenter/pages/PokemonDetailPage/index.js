...
import MultiSelector from 'features/PokemonDetail/presenter/components/MultiSelector'


const PokemonDetailPage = () => {
    const { id } = useParams()
    const [pokemonNameList, setPokemonNameList] = useState([])
    const [pokemonNamesSelected, setPokemonNamesSelected] = useState([])
        ...
        () => {
            ...
            const getPokemons = async() => {
                ...
                if (response && response.data && response.data.results.length > 0) {
                    var names = response.data.results.map(function(item) {
                        return item['name'];
                    });
                    setPokemonNameList(names)
                }
            }
    ...
    const handleChangeNames = (e) => {
        setPokemonNamesSelected(e)
    }

    const handleCompare = () => {
        navigate('/compare/' + id + '--' + pokemonNamesSelected.join('--'))
    }
            ...
            <Grid container justifyContent="center" spacing={4} marginBottom={3} marginTop={1}>
                <Box xs={12} display={'flex'} justifyContent={'center'}>
                    <Grid item xs={4} alignItems="center">
                        <img className="animated infinite pulse delay-2s"  src={pokemonDetail.sprites && pokemonDetail.sprites.other && pokemonDetail.sprites.other.home && pokemonDetail.sprites.other.home.front_default} width={170} />
                    </Grid>
                    <Grid item xs={8} >
                ...
                <Grid item xs={12} display={'flex'} justifyContent={'center'}>
                    <Grid  justifyContent={'center'} padding={1} sx={{fontFamily:'Monserrat , sans-serif', background: 'linear-gradient(to bottom, #f0f7f5 0%, #cbfac5 300%)', borderRadius: '17px', width: '550px', overflow: 'hidden', marginTop: '10px', marginBottom: '7px'}}>
                        <Typography sx={{marginBottom: '6px', fontSize: '21px', color: '#44696c', fontWeight: 600, textTransform: 'uppercase' }}>
                            Compare {pokemonDetail.name} with 50 Pokémons
                        </Typography>
                        <MultiSelector
                            nameList={pokemonNameList}
                            selectedNames={handleChangeNames}
                        />
                        <Button onClick={handleCompare} sx={{background: 'linear-gradient(to right, #a2db95 0%, #6dedb7 100%)', borderRadius: '18px'}} variant="contained" disabled={!(pokemonNamesSelected.length > 0)}>Compare {pokemonDetail.name}</Button>
                    </Grid>
                </Grid>
            </Grid>
        </Container>
